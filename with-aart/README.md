...
1. **The AART MCP server, wired per workspace** (`aart init-agent`, invoked for you) — `aart_find_blocks`, `aart_verify`, `aart_register_block`, `aart_validate`, `aart_run_workflow`, `aart_get_report`, and more (17 tools on a bare-fresh, governed-mode project — more unlock as the workspace registers environments, eval suites, and remotes). This step is **per workspace** by design — AART's store is workspace-scoped, like `.git`, so there's no single "install once for every project" step for the MCP wiring itself.
