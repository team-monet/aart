...
describe("SQLite adapter — connection setup (architecture §5.1)", () => {
  ...
  it("auto-runs migrations by default, advancing the watermark to the latest ordinal (3, since D2a's 0003_approval_task_authenticated_as — AMENDMENTS.md A59; was 2 as of D1's 0002_deployment_promoted, AMENDMENTS.md A56)", async () => {
  ...
  it("MigrationRunner can be driven explicitly against a runMigrations:false store, and is idempotent", async () => {
    ...
    try {
      ...
      // D1 (AMENDMENTS.md A56) + D2a (AMENDMENTS.md A59): three migrations
      // now registered (0001_init, 0002_deployment_promoted,
      // 0003_approval_task_authenticated_as) — up() from watermark 0
      // applies all three in one call, landing on the latest ordinal.
