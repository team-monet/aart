...
1. **Discover reusable assets.** Call \`aart_find_workflows\` first. Then use \`aart_find_blocks\` (by query) or \`aart_list_blocks\` (the full catalog) to see what you can compose. If local search misses, call \`aart_find_packs\`; \`aart_install_pack\` downloads a match inertly as **unapproved**, and \`aart_list_packs\` shows its exact provenance/hash before you ask the user to approve it. Never call \`aart_approve_pack\` without explicit human approval. \`aart_propose_workflow\` returns a ready-made recipe skeleton if your request matches one of the built-in patterns (verify a page, check API health, wait for approval, ...) — check these before composing from scratch.
